...
SnakeMap::SnakeMap(int lenx,int leny,double xyr){
    xw=lenx;yw=leny;xyreso=xyr;
    width_x=xw*xyreso;
    width_y=yw*xyreso;
    pmap.setIdentity(xw,yw);
    pmap=pmap/2.0;
    logm.setZero(xw,yw);
    minx=-width_x/2.0;maxx=-minx;
    miny=-width_y/2.0;maxy=-miny;
...
int SnakeMap::update(Eigen::VectorXd ox,Eigen::VectorXd oy,double center_x,double center_y){
    ...
    for (int i=0;i<ox.size();i++){
        x=getindexX(ox(i));y=getindexY(oy(i));
        gridset(x,y,Occupy);
        bresenham(cx,cy,x,y);
    }
}
...
int main(int argc,char **argv){
    ...
    for (int i=0;i<=20;i++) ox(i)=(i-10)/10.0,oy(i)=ox(i)/2+0.5;
    map_test.update(ox,oy,0,0);
    for (int i=0;i<=20;i++) ox(i)=(i-10)/10.0,oy(i)=-ox(i)/2-0.5;
    map_test.update(ox,oy,0,0);
    for (int i=0;i<=20;i++) ox(i)=0.9,oy(i)=(i-10)/10.0;
    map_test.update(ox,oy,0,0);
